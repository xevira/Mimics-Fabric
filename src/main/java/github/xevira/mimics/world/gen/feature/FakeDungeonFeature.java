package github.xevira.mimics.world.gen.feature;
...
// Pretty much thee DungeonFeature class, just replacing the chest and spawner
public class FakeDungeonFeature  extends Feature<DefaultFeatureConfig> {
    ...
    public FakeDungeonFeature(Codec<DefaultFeatureConfig> codec) {
        super(codec);
    }
    ...
    public boolean generate(FeatureContext<DefaultFeatureConfig> context) {
        ...
        if (r >= 1 && r <= 5) {
                ...
                for (int t = 3; t >= -1; t--) {
                    for (int u = p; u <= q; u++) {
                        ...
                        if (s == k || t == -1 || u == p || s == l || t == 4 || u == q) {
                            ...
                            } else if (blockState.isSolid() && !blockState.isOf(Registration.MIMIC_CHEST)) {
                        ...
                        } else if (!blockState.isOf(Registration.MIMIC_CHEST) && !blockState.isOf(Registration.FAKE_SPAWNER)) {
                    ...
                    if (structureWorldAccess.isAir(blockPos3)) {
                        ...
                        if (x == 1) {
                            this.setBlockStateIf(
                                    structureWorldAccess, blockPos3, StructurePiece.orientateChest(structureWorldAccess, blockPos3, Registration.MIMIC_CHEST.getDefaultState()), predicate
                            );
                            break;
                        }
                    }
            ...
            this.setBlockStateIf(structureWorldAccess, blockPos, Registration.FAKE_SPAWNER.getDefaultState(), predicate);
            if (structureWorldAccess.getBlockEntity(blockPos) instanceof FakeSpawnerBlockEntity mobSpawnerBlockEntity) {
                mobSpawnerBlockEntity.setEntity(this.getMobSpawnerEntity(random));
            } else {
                LOGGER.error("Failed to fetch mob spawner entity at ({}, {}, {})", blockPos.getX(), blockPos.getY(), blockPos.getZ());
            }

            // Keep this around for now
            //Mimics.LOGGER.info("FakeDungeon generated at {}", blockPos);
            return true;
        } else {
            return false;
        }
